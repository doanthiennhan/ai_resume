...
connection = pymysql.connect(host='localhost', user='root', password='Nhan2792003@')
...
def run():
    ...
    st.image(img)
    # Thêm nút "Create CV" vào header
    st.markdown(
    '<a href="http://localhost:5173/" target="_self" style="display: inline-block; padding: 10px 20px; font-size: 16px; color: white; background-color: #007bff; border-radius: 5px; text-decoration: none;">Create CV</a>',
    ...
    if choice == 'Normal User':
        ...
        if st.button('Login'):
            if ad_user == 'nhan' and ad_password == '123':
